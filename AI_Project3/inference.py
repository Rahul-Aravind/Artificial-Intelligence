        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDict.items():
                updatedBeliefs[newPos] += prob * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for index in range(self.numParticles):
            self.particles.append(self.legalPositions[index % legalPosLen])
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ...
            updatedBeliefs.append(util.sample(newPosDist))

        self.particles = updatedBeliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        legalGhostsPositions = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(legalGhostsPositions)

        #print "num ghosts, ", self.numGhosts
        #print "Permutations, ", legalGhostsPositions
        #print "Permutations len, ", len(legalGhostsPositions)

        self.particles = []
        legalGhostsPosLen = len(legalGhostsPositions)

        for index in range(self.numParticles):
            self.particles.append(legalGhostsPositions[index % legalGhostsPosLen])
    ...
    def observeState(self, gameState):
        ...
        for ghostIdx in range(self.numGhosts):
            if noisyDistances[ghostIdx] is None:
                killedGhosts.append(ghostIdx)
                for index in range(self.numParticles):
                    self.particles[index] = self.getParticleWithGhostInJail(self.particles[index], ghostIdx)

        allPossible = util.Counter()
        for particle in self.particles:
            prob = 1
            for ghostIdx in range(self.numGhosts):
                if ghostIdx not in killedGhosts:
                    TrueDistance = util.manhattanDistance(particle[ghostIdx], pacmanPosition)
                    prob *= emissionModels[ghostIdx][TrueDistance]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for ghostIdx in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, prevGhostPositions), ghostIdx, self.ghostAgents[ghostIdx])
