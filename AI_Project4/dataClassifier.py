...
def enhancedFeatureExtractorDigit(datum):
    ...
    #print DIGIT_DATUM_WIDTH
    #print datum
    #print datum.getPixels()

    #print "features, ", features

    whiteRegions = findRegions(datum, 1)
    features["one_white_region"] = whiteRegions % 2
    features["two_or_three_white_region"] = (whiteRegions / 2) % 2
    features["four_white_region"] = (whiteRegions / 4) % 2

    for x in xrange(1, DIGIT_DATUM_WIDTH):
        for y in xrange(1, DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x, y) == 0:
                features[("horizontal_line", x, y)] = 0
                features[("vertical_line", x, y)] = 0
            else:
                features[("horizontal_line", x, y)] = int(datum.getPixel(x, y) > datum.getPixel(x - 1, y))
                features[("vertical_line", x, y)] = int(datum.getPixel(x, y) > datum.getPixel(x, y - 1))

    # util.raiseNotDefined()

    return features

#performing dfs to find the number of connected regions

def findRegions(datum, component):
    visited = []
    connectedComponents = 0
    for x in xrange(DIGIT_DATUM_WIDTH):
        for y in xrange(DIGIT_DATUM_HEIGHT):
            if (x, y) not in visited and datum.getPixel(x, y) <= component:
                connectedComponents += 1
                recursion = [(x, y)]
                while recursion:
                    coordinate = recursion.pop()
                    visited.append(coordinate)
                    for point in getNeighbors(coordinate[0], coordinate[1]):
                        if point not in visited and datum.getPixel(point[0], point[1]) <= component:
                            recursion.append(point)
...
def enhancedPacmanFeatures(state, action):
    ...
    pacman_pos = successorState.getPacmanPosition()
    foods = successorState.getFood().asList()
    closestFood = None
    ...
    if closestFood is None or closestFood == 0:
        features["closest_food_distance"] = 0
    else:
        features["closest_food_distance"] = (5.0 / closestFood) * 2.5

    features["foods_left"] = state.getNumFood()

    ghosts = successorState.getGhostPositions()
    closestGhost = None

    capsules = successorState.getCapsules()
    features["capsules"] = len(capsules)

    if ghosts:
        for ghost in ghosts:
            distance = util.manhattanDistance(ghost, pacman_pos)
            if closestGhost is None or distance < closestGhost:
                closestGhost = distance

    if closestGhost is None or closestGhost == 0:
        features["closest_ghost_distance"] = 0
    else:
        features["closest_ghost_distance"] = 5.0 / closestGhost

    features["successor_state_score"] = successorState.getScore()
